...
function cellHighlight(x) {
	...
	if (!x.querySelector("h1")) {
		...
		var x2r = x.querySelector("h1");

		function shs() {
			if (x2r.innerHTML == h1txt) {
				x2r.remove()
			}
		}
	}

	setTimeout(() => [x.style.backgroundColor = "", x.classList.remove("clicked")], 22000);
	setTimeout(() => [shs()], 22000);
}
