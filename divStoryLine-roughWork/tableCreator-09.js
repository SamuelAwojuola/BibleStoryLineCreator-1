...
function cellHighlight(x) {
	...
	setTimeout(() => [x.style.backgroundColor = "", x.classList.remove("clicked")], 25000)
}
...
/*ADD HEADING TO CLICKED CELL***************************************************************************/
	var input = document.getElementById("cellHeading"); //GETS VALUE FROM INPUT BOX
	...
	input.addEventListener("keyup", function (event) {
		...
		if (event.keyCode === 13) {
			...
			document.getElementById("applyHeadingBtn").click();
		}
	});
function insertHeading() {
	var x = input.value; //GETS VALUE FROM INPUT BOX
	...
	if (x) {
		...
		document.getElementById("cellHeading").value = "";
	}
